...
export default function RisetPengembangan() {
  const articles = [
    {
      id: 1,
  ...
  return (
    <section className="container py-20 mx-auto font-poppins bg-slate-100 lg:py-30 max-w-7xl">
      ...
      {/* Grid Artikel */}
      <div className="grid gap-6 py-20 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5">
        {articles.map((article) => (
          <div key={article.id} className="relative overflow-hidden shadow-md rounded-3xl group">
            {/* Gambar */}
            <img src={article.image} alt={article.title} className="object-cover w-full transition-transform duration-500 h-80 group-hover:scale-105" />
            ...
            <div className="absolute bottom-0 left-0 right-0 p-4 text-white bg-gradient-to-t from-black/70 to-transparent">
              ...
              <h3 className="text-xs font-semibold sm:text-base">{article.title}</h3>
